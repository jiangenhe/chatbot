...
class TextStep extends Component {
  ...
  renderMessage = () => {
    ...
    return <Twemoji text={this.getMessage()} />;
  };
    ...
    return (
      <TextStepContainer className={`rsc-ts ${user ? 'rsc-ts-user' : 'rsc-ts-bot'}`} user={user}>
        <ImageContainer className="rsc-ts-image-container" user={user}>
          ...
          {name && isFirst && showAvatar && (
            <div style={{display: "flex", justifyContent: 'center', fontSize: '12px', fontWeight: 'bold'}}>{name}</div>
          )}
        </ImageContainer>
        <Bubble
